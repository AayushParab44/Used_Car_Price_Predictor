...
with col1:
    wheel_system = st.selectbox("wheel_system", ['FWD', 'AWD', '4WD', 'RWD' ,'4X2'])
    body_type = st.selectbox("body_type", ['Sedan' ,'Coupe' ,'SUV / Crossover', 'Pickup Truck', 'Wagon', 'Minivan','Convertible', 'Hatchback', 'Van'])
    fuel_type = st.selectbox("fuel_type", ['Gasoline', 'Flex Fuel Vehicle', 'Diesel', 'Hybrid', 'Biodiesel','Compressed Natural Gas', 'Propane'])
    ...
    engine_cylinders =  st.selectbox("engine_cylinders",['I4', 'V6', 'V8', 'H4', 'I6', 'V8 Flex Fuel Vehicle', 'V6 Flex Fuel Vehicle','I4 Diesel', 'I6 Diesel', 'I5' ,'I3', 'I4 Hybrid' ,'I4 Flex Fuel Vehicle',
    ...
    make_name =  st.selectbox("make_name", ['Chevrolet', 'Lexus', 'Jeep', 'Hyundai', 'Cadillac', 'Chrysler', 'Dodge','Nissan', 'Honda', 'Kia' ,'Ford', 'Lincoln', 'Subaru' ,'BMW' ,'Audi',
 ...
 'Scion', 'Porsche', 'Saturn', 'Maserati', 'Pontiac', 'Mercury' ,'Oldsmobile', 'Aston Martin' ,'Suzuki', 'Isuzu', 'Hummer', 'Plymouth'])
    ...
    listing_color  =  st.selectbox("listing_color",  ['SILVER', 'BLACK', 'RED', 'WHITE', 'UNKNOWN', 'BLUE', 'GRAY', 'BROWN', 'YELLOW', 'ORANGE', 'GREEN' ,'PURPLE', 'TEAL' ,'GOLD', 'PINK'])
...
with col2:
    ...
    city_fuel_economy  = st.number_input("city_fuel_economy ", min_value=1.0, max_value=100.0)
    highway_fuel_economy  = st.number_input("highway_fuel_economy ", min_value=1.0, max_value=100.0)
    wheelbase  = st.number_input("wheelbase ", min_value= 70.0, max_value=180.0)
    length  = st.number_input("length ", min_value=120.0, max_value=280.0)
    engine_displacement  = st.number_input("engine_displacement ", min_value=1000.0, max_value=9000.0)
...
df = pd.get_dummies(user_data, columns=['wheel_system','body_type','fuel_type','maximum_seating','engine_cylinders','make_name','listing_color'],dtype=int)
